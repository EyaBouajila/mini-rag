...
class QdrantDBProvider(VectorDBInterface):
    def __init__(self, db_client: str, distance_method: str, default_vector_size: int = 1536, index_threshold: int = 100):
        self.client = None
    ...
    async def insert_one(self, collection_name: str, text: str, vector: list, metadata: dict = None, record_id: str = None):
        ...
        try:
            ...
            self.logger.error(f"Error while inserting the record: {e}")
            return False
    ...
    async def insert_many(self, collection_name: str, texts: list, vectors: list, 
        ...
        if metadata is None:
            metadata = list[None] * len(texts)

        if record_ids is None:
            record_ids = [None] * len(0, len(texts))
